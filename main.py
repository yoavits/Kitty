...
class ButtonSettingView(View):
    ...
    @discord.ui.button(label="Set Custom values")
    async def set_custom_values(self, button: discord.ui.Button, interaction: discord.Interaction):
        modal_view = ModalView("Button Settings", self.user_id, interaction)
        await interaction.response.edit_message(view=modal_view)

class ModalView(View):
    def __init__(self, title: str, user_id: int, interaction: discord.Interaction):
        super().__init__()
        self.title = title
        self.user_id = user_id
    ...
    async def prompt_for_input(self, interaction: discord.Interaction, prompt: str) -> str:
        await interaction.response.send_message(prompt, ephemeral=True)
        message = await self.bot.wait_for('message', check=lambda m: m.author == interaction.user and m.channel == interaction.channel)
        return message.content

    @discord.ui.button(label="Confirm", style=discord.ButtonStyle.green)
    async def confirm_modal(self, button: discord.ui.Button, interaction: discord.Interaction):
        label = await self.prompt_for_input(interaction, "What text should the button display?")
        emoji_id = await self.prompt_for_input(interaction, "What is the ID of the custom emoji you would like to use (if any)?\n\n*This is optional*")
        link = await self.prompt_for_input(interaction, "What is the URL that the button should link to?")

        confirm_embed = discord.Embed(
            title=self.title,
            description=f"Button's type: Link Button\n\nSet label to: {label}\nSet emoji to: {emoji_id}\nSet link to: {link}",
            color=self.color
        )

        confirm_message = await interaction.response.send_message(embed=confirm_embed, components=[])
        ...
        if emoji_id is not None and emoji_id.strip():
            link_button.emoji = discord.PartialEmoji(id=int(emoji_id))

        message = await interaction.channel.fetch_message(self.interaction.message.reference.message_id)
        await message.edit(components=message.components + [[link_button]])

        # Send the confirmation message and clear components from the original message
        await interaction.followup.send(embed=discord.Embed(title=self.title, description="Button added successfully", color=self.color))
        await self.interaction.message.edit(components=[])

        await confirm_message.delete()


@tree(name="add_button", description="Add button to your message")
#@cooldown(10, 60.0, commands.BucketType.user)
async def add_button(interaction: discord.Interaction, message_link: str):
    try:
        message_link = int(message_link.split("/")[-1])
        message = await interaction.channel.fetch_message(message_link)
    except (ValueError, discord.NotFound, discord.HTTPException):
        return await interaction.response.send_message("Failed to fetch the message with the provided link.")

    view = ButtonSettingView(interaction.user.id)
    await interaction.response.send_message(
        "Choose a custom label, emoji, and link with the popup form.\n\n",
        view=view
    )
...
"""
# Command for verify members ✅
@tree(name="verify", description="Verify that you are not a robot ✅")
@cooldown(3, 60.0, key=lambda i: str(i.user.id))
@access(GLOBAL_ACCESS_ID)
async def verify(interaction: discord.Interaction):
    # Check if the user already has the desired role
    role_id_added = 1069003932363391016
    role_id_removed = 1091471850095251618
    role_added = interaction.guild.get_role(role_id_added)
    role_removed = interaction.guild.get_role(role_id_removed)

    await interaction.response.defer(thinking=False, ephemeral=True)

    if not role_added:
        await interaction.followup.send(f"{ERROR_EMOJI} Sorry, the verification role does not exist on this server.", ephemeral=True)
        return
    elif role_added in interaction.user.roles:
        await interaction.followup.send(f"{ERROR_EMOJI} You have already been verified.", ephemeral=True)
        return

    # generate a simple math problem (addition or subtraction)
    num1 = random.randint(1, 10)
    num2 = random.randint(1, 10)
    operation = random.choice(["+", "-"])
    if operation == "+":
        problem = f"What is the result of {num1} + {num2}?"
    ...
    user_answer = user_response.content.lower().strip().rstrip("?!.")  # normalize the answer
    user_answer = re.sub(r"\s+", "", user_answer)  # remove all whitespace characters
    if user_answer.isdigit() and int(user_answer) == correct_answer:
        # give the user the desired role
        if role_removed in interaction.user.roles:
            await interaction.user.remove_roles(role_removed)
            
        await interaction.user.add_roles(role_added)
        #await user_response.reply(f" {SUCCESS_EMOJI} Congratulations, you passed the verification process!", mention_author=False)
        await interaction.followup.send(f"{SUCCESS_EMOJI }Congratulations!  you passed the verification process!", ephemeral=True)
    elif str(correct_answer) in user_answer:
        # give the user the desired role
        if role_removed in interaction.user.roles:
            await interaction.user.remove_roles(role_removed)
        ...
        #await user_response.reply(f" {ERROR_EMOJI} Sorry, your answer was incorrect. Please try again.", mention_author=False)
        await interaction.followup.send(f"{ERROR_EMOJI} Sorry, your answer was incorrect. Please try again.", ephemeral=True)
...
# Dictionary to store @cooldown time remaining for each command and user
@cooldowns = {}

# Commands Errors Embed ⚠️
@bot.tree.error
async def on_slash_command_error(interaction: discord.Interaction, error: commands.CommandError):
    error_embed = creating_error_embed()
    time_remaining = 0  # Assign an initial value to time_remaining
    if isinstance(error, app_commands.CommandOnCooldown):
        # Command Cooldown
        time_remaining = int(error.retry_after)
        @cooldown_timestamp = datetime.datetime.now() + datetime.timedelta(seconds=time_remaining)
        epoch_time = int(@cooldown_timestamp.timestamp())
        command_name = interaction.data["name"]
        user_id = interaction.user.id

        # Check if user already has an active @cooldown for this command
        if command_name in @cooldowns and user_id in @cooldowns[command_name]:
            time_remaining = @cooldowns[command_name][user_id]

        error_embed.title = "Too fast! <:error:1088811536497254470>"
        error_embed.description = f"This command is on @cooldown. \n Please try again in <t:{epoch_time}:R>"
        ...
        async def update_embed():
            ...
            ##print("update_embed function started")
            if time_remaining <= 0:
                return
            while time_remaining > 0:
                ...
                time_remaining -= 1
                if command_name in @cooldowns and user_id in @cooldowns[command_name]:
                    @cooldowns[command_name][user_id] = time_remaining
                if time_remaining <= 0:
                    await interaction.delete_original_response()
                    if command_name in @cooldowns and user_id in @cooldowns[command_name]:
                        @cooldowns[command_name].pop(user_id, None)
                else:
                    error_embed.description = f"This command is on @cooldown. \n Please try again in <t:{epoch_time}:R>"
                    #print(f"updated embed to {error_embed.description}")
                    await interaction.edit_original_response(embed=error_embed)

        # Add the @cooldown to the dictionary
        ...
        print(f"There is an error in the code: {error}")
